...
public class RampageEnemyCtrl : MonoBehaviour
{
    // 体力を指定する変数
    public int health;
    // ダメージのチェック
    ...
    // 移動方向切り替えスイッチ用(trueならx軸上falseならy軸上を移動)
    public bool ChangeCheck = true;
    ...
    void Update()
    {
        ...
        if (!damagecheck)
        {// ダメージを受けていない間
            damage.color = new Color(0.0f, 0.0f, 1.0f, 1.0f);
        }
        else
        {// ダメージを受けている間
    ...
    void SlidmvEnemy()
    {
        ...
        if (CheckPosition)
        {
            transform.Translate(speed_ * 0.02f, 0.0f, 0);
        }
        else
        {
            transform.Translate(-speed_ * 0.02f, 0.0f, 0);
